...
export const LoginPage: React.FC = () => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const loginRes = await authService.signIn(data.email, data.password);
      stopImpersonation();
      setUser(loginRes.user);
      navigate(resolvePostLoginRedirect(fromPath, loginRes.user.role), { replace: true });
    } catch (err: unknown) {
        ...
        >
          ...
          <div className="relative z-10 max-w-sm space-y-6">
            <div className="space-y-1">
              <p className="text-xs tracking-caps text-primary-foreground/50">Delegate Portal</p>
            ...
            <p className="text-sm leading-relaxed text-primary-foreground/70">
              Access your registration status, committee assignment, and conference credentials from your
              personal delegate portal.
            </p>
          </div>
        ...
        <div className="flex flex-col justify-center px-6 py-12 sm:px-10 lg:px-16 xl:px-24">
          ...
          >
            ...
            <form onSubmit={onSubmit} className="space-y-5" noValidate>
              ...
              <Button type="submit" className="h-11 w-full font-semibold" isLoading={isSubmitting}>
                {!isSubmitting && (
                  <>
                    Sign in
