...
export const ChairAttendancePage: React.FC = () => {
  const [searchQuery, setSearchQuery] = useState("")
  const [activeTab, setActiveTab] = useState("search")

  // Fetch active period
  const { data: activePeriod, isLoading: isLoadingPeriod } = useQuery({
    queryKey: ["chair", "attendance", "active"],
    queryFn: () => chairDashboardService.getActivePeriod(),
  })

  // Search delegates (filtered by council on backend)
  const { data: searchResults, isFetching: isSearching } = useQuery({
    queryKey: ["chair", "delegates", "search", searchQuery],
    queryFn: async () => {
      if (searchQuery.trim().length < 2) return []
      const res = await chairDashboardService.searchDelegates(searchQuery)
      return res // searchDelegates returns the array directly from api.get
    },
  ...
  const recordAttendance = useMutation({
    mutationFn: async (delegateId: string) => {
      ...
      return await chairDashboardService.recordAttendance({
        delegateId,
  ...
  return (
    <ChairDataPageLayout
      ...
      description="Manage delegate attendance for the current session."
            ...
            <TabsContent value="search" className="m-0 space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <AnimatePresence mode="popLayout">
                  {searchResults?.map((delegate: any) => (
                    <motion.div
                      key={delegate.userId}
                ...
                {searchQuery.length < 2 && (
                  <div className="col-span-full border border-dashed rounded-xl py-20 text-center bg-muted/10">
                    ...
                    <p className="mt-4 text-sm text-muted-foreground/60">Type to search for delegates in your council</p>
                  </div>
                )}
              </div>
            </TabsContent>
