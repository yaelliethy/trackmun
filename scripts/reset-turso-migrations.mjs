 ...
 * Reads TURSO_DATABASE_URL and TURSO_AUTH_TOKEN from repo root .dev.vars.
 */
...
const DEV_VARS = join(REPO_ROOT, '.dev.vars');
...
async function main() {
  ...
  if (!url || !authToken) {
    console.error(
      'reset-turso-migrations: TURSO_DATABASE_URL and TURSO_AUTH_TOKEN must be set in .dev.vars at repo root',
    );
    process.exit(1);
  }
