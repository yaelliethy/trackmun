...
import { decodeJwtPayloadJson } from '../lib/verify-supabase-jwt';
  ...
  if (parts.length === 3) {
    ...
    if (payload?.typ === 'impersonation') {
      ...
      if (isValid && typeof payload.actingAs === 'string') {
        const user = await fetchUserFromTurso(payload.actingAs);
        if (user) {
          c.set('user', user);
      ...
      if (typeof sub === 'string') {
        ...
        if (!user) {
          // Log DB fallback
          user = await fetchUserFromTurso(sub);
...
async function fetchUserFromTurso(userId: string): Promise<User | null> {
  ...
  try {
    ...
    if (!user) {
      console.warn(`fetchUserFromTurso: User not found in Turso for ID: ${userId}`);
      return null;
    }
    ...
    console.error(`fetchUserFromTurso: Database error for ID: ${userId}`, err);
    return null;
  }
}
