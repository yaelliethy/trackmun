...
export type InitializeDbOptions = TursoConfig & {
  /** When false (e.g. production), SQL is not logged to the console. */
  logQueries?: boolean;
};

export function initializeDb(config: InitializeDbOptions) {
  if (dbInstance) {
    return dbInstance;
  }
  ...
  try {
    ...
    const shouldLog = config.logQueries === true;
    clientInstance = shouldLog ? wrapClient(rawClient) : rawClient;
