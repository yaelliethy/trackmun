...
routes.openapi(createRoute({
  method: 'get', path: '/responses/{userId}', request: { params: z.object({ userId: z.string() }) }, responses: { 200: { description: 'Get delegate responses', content: { 'application/json': { schema: z.object({ success: z.literal(true), data: DelegateResponseSchema }) } } }, 404: { description: 'Not found', content: { 'application/json': { schema: z.object({ success: z.literal(false), error: z.string() }) } } } }
}), registrationController.getResponse);
